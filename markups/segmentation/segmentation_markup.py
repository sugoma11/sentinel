...
if not os.path.isdir(f'../data/raw/{PORT_NAME}/marked'):
    os.mkdir(f'../data/raw/{PORT_NAME}/marked')

if len(list(Path(f'../data/raw/{PORT_NAME}/marked').glob('*.png'))) == 0:
    num = 0

else:
    num = len((sorted(Path(f'../data/raw/{PORT_NAME}/marked').glob('*.png'), key=os.path.getmtime)))
